 ...
 * We are finding the bottom right-most node (which is a leaf)
 *	and removing that one
 */
int drop_one_node  () {
    // Your code here
    ...
    printf("Node key: %s\n", node->key);
    printf("concat: %s\n", concat);
    int result = delete (concat, concatlen);					//call delete and the returned int goes into variable result
